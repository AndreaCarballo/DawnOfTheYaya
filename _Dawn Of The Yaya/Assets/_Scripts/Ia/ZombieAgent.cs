...
public class ZombieAgent : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        bool inView = false;
        // Checks is player is in the zombie's field of view
        ...
        if (targetsInViewRadius.Length != 0)
        {
            ...
            if (Vector3.Angle(transform.forward, dirToTarget) < FOVAngle / 2)
            {
                ...
                if (!Physics.Raycast(transform.position, dirToTarget, dstToTarget, obstacleLayer))
                {
                    inView = true;
                    seen = true;
                    // activates player in FOV behavior
        ...
        if (!inView)
        {
            // If the player is not in sight, it checks if the zombie can hear anything
            ...
            if (heard) // If the player was heard
            {
                AgentBehaviorHearing();
            }
            else if (seen) // If the player was seen in the past
            {
                AgentBehaviorSight(false);
            }
            else // If the zombie lost the player, it looks around for a short period of time
            {
                if (turn)
                {
                    if (countTurn <= 700)
                ...
                else // then it goes back to patrolling
                {
                    Patrol();
                }
    ...
    void LateUpdate()
    {
        // If the difficulty is set to Hard, the zombies' fields of view are not drawn
    ...
    void AgentBehaviorSight(bool inView)
    {
        if (inView)
        {
            // If the player was heard before it was seen, the sound type heard gets a positive reward
            if (heard)
            {
            ...
            // The positions need to be reduced to ranges to help the matrix converge
            ...
            // We check if the zombie is too close to other zombies, either left or right
            ...
            // If the zombie is not too close to another enemy anymore, it gets a positive reward
            if (zombieRightPast && !zombieRight)
            {
                currentReward += distanceReward * 2;
            }
            // It if it still is, it gets a negative one
            ...
            if (currentDistance < lastDistance) //if the agent got closer to the player it'll get a positive reward
            {
            ...
            else //if it didn't it'll get a negative reward
            {
                currentReward += timePunishment;
            }
        ...
        if (seen && !inView)
        {
            // If the player was seen but it's not in sight anymore, it gets a negative reward
            currentReward += negativeReward;
            float distSee = Vector3.Distance(lastPosition, transform.position);
            if (distSee > 1f) // it walks to the player's last known position
            {
            ...
            else // if it gets there without seeing the player, it starts to look around
            {
                anim.SetTrigger("Idle");
    ...
    void AgentBehaviorHearing()
    {
        ...
        if (distHear > 1f)
        {
            // The agent decided whether to go or not for each new sound it hears
            ...
            // It keeps going to the position where it heard the sound
            if (go == 1)
            {
        ...
        // If the player isn't there, it gets a negative reward and starts to look around
        else
        {
            gotHearingAction = false;
            lastSoundType = 0;
